 ...
 */
export async function extractProductWithHyperAgent(url: string): Promise<ProductDNA> {
  ...
  console.log(`[HyperAgent] 🌐 Starting extraction for: ${finalUrl} (${domain})`);
  ...
  try {
    ...
    // Filter out common non-product content patterns
    ...
    for (const line of lines) {
      ...
      // Reset skip flag when we see product-related content
      if (skipSection && (
        line.includes('price') || 
        line.includes('₹') || 
        line.includes('INR') ||
    ...
    if (aiFriendlyContent.length > MAX_CONTENT_SIZE) {
      ...
      console.log(`[HyperAgent] 📊 Accessibility tree: ${(originalSize / 1024).toFixed(2)}KB → ${(aiFriendlyContent.length / 1024).toFixed(2)}KB (filtered & truncated)`);
    } else {
      console.log(`[HyperAgent] 📊 Accessibility tree: ${(originalSize / 1024).toFixed(2)}KB → ${(aiFriendlyContent.length / 1024).toFixed(2)}KB (filtered)`);
    }
    ...
    const productSchema = z.object({
      ...
      brand: z.string().describe("The brand/manufacturer name (e.g., Apple, Samsung, Nike). NEVER use domain names or 'www'"),
...
Page content (accessibility tree):\n${aiFriendlyContent}\n\nExtract the information for the MAIN PRODUCT ONLY and return it as JSON matching the requested schema.`;
    ...
    console.log(`[HyperAgent] 💰 Estimated tokens: ~${estimatedTokens.toLocaleString()} (${aiFriendlyContent.length} chars)`);
    ...
    console.log(`[HyperAgent] ✅ Extraction complete: ${productDNA.name} - ${productDNA.price} ${productDNA.currency}`);
    ...
    try {
      if (!page.isClosed()) {
        await page.close();
        console.log(`[HyperAgent] 🗑️ Page closed for: ${finalUrl}`);
      }
    } catch (closeError: any) {
      // Ignore errors if page is already closed - this is harmless
      if (!closeError.message?.includes('Target closed') && 
          !closeError.message?.includes('Session closed')) {
        console.warn(`[HyperAgent] ⚠️ Failed to close page:`, closeError.message);
      }
    }
  }
}
